...
//               mexPrintf("Prima della ricerca");
            ...
            Tree->SearchKClosest(&pstruct[0],&pk,idc,distances,kint);//lancio la routine per la ricerca del più vicino
//            mexPrintf("Dopo la ricerca");
    ...
    else//Ricerca con distanze
    {
        ...
        for (i=0;i<Nq;i++)
        {
            ...
            Tree->SearchKClosest(pstruct,&pk,idc,distances,kint);//lancio la routine per la ricerca del più vicino
            for (j=0;j<kint;j++)
            {
                idcdouble[Nq*(j)+i]=idc[j]+1;//convert to matlab notation
